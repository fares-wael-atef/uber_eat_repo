/**
 * dashboard.js v7 — Main controller for Ali Baba's Shawarma Dashboard
 * Dataset Source: /Users/mac/Downloads/aly-baba (48 reporting files)
 * 100% DYNAMIC ENGINE:
 * Dynamically re-calculates KPI counters, charts, topbar badges, and strategic insight text
 * whenever Date Period, Branch, or Channel filters are changed!
 */
...
(function () {
  ...
  function initChartsForSection(section) {
    switch (section) {
      ...
      case 'calendar':  window.CalendarManager.init();       break;
    }
  }
  ...
  function updateDynamicInsights() {
    ...
    if (bgOverview) {
      bgOverview.innerHTML = `
        ...
        For <strong>${periodLbl}</strong> (${activeBranch}), Ali Baba's Shawarma generated <strong>CAD $${parseFloat(totals.totalRevenue).toLocaleString()} in net payout revenue</strong> across <strong>${totals.totalOrders.toLocaleString()} total orders</strong>. Top revenue contributor is <strong>${topStore.name}</strong> (<strong>CAD $${topStore.payout.toLocaleString()}</strong> net payout over ${topStore.orders.toLocaleString()} orders). Network customer rating holds at <strong>${totals.avgRating} / 5.0 ★</strong>.
      `;
    }
    ...
    if (bgOrders) {
      bgOrders.innerHTML = `
        <strong style="font-size:0.92rem;">Peak Volume & Delivery Fulfillment Insights (${periodLbl}):</strong><br>
