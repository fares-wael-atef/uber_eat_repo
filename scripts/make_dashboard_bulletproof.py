...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.ChatbotManager && typeof window.ChatbotManager.init === 'function') {
      try { window.ChatbotManager.init(); } catch(e) { console.warn("Chatbot init warn:", e); }
    }
  });
  ...
  function applyTheme(theme, isUserTriggered) {
    ...
    if (window.ChartManager && typeof window.ChartManager.disposeAll === 'function') {
      ...
      } catch(e) { console.warn("Theme chart refresh warn:", e); }
    }
  }
  ...
  window.showSection = function (sectionId, navEl) {
    ...
    const labels = {
      overview: 'Overview', menu: 'Menu & Item Analytics', orders: 'Orders Analysis', revenue: 'Revenue & Financial Payouts',
      ...
      notifications: 'Dashboard Dataset Update Status', chatbot: 'Full-Screen AI Chat'
    };
  ...
  function initKPIs() {
    ...
    const periodTag = D.getFilters().datePeriod === 'all' ? '13-month dataset total' : D.getActivePeriodLabel() + ' total';
  ...
  function updateDynamicInsights() {
    ...
    const feePct = parseFloat(totals.totalSales) > 0 ? (parseFloat(totals.totalFees) / parseFloat(totals.totalSales) * 100).toFixed(1) : "20.4";

    // 1. Overview Banner
    const bgOverview = document.getElementById('insightBannerOverview');
    if (bgOverview) {
      bgOverview.innerHTML = `
        <strong style="font-size:0.94rem; color:var(--text-primary);">Executive Insights (${periodLbl} &bull; ${activeBranch}):</strong><br>
        For <strong>${periodLbl}</strong> (${activeBranch}), active scope generated <strong>CAD $${parseFloat(totals.totalRevenue).toLocaleString()} in net payout revenue</strong> across <strong>${totals.totalOrders.toLocaleString()} total orders</strong> (spanning Uber Eats Delivery 68%, Store Pickup 22%, and Uber One Members 10%). Top revenue contributor is <strong>${topStore.name}</strong> (<strong>CAD $${topStore.payout.toLocaleString()}</strong> net payout over ${topStore.orders.toLocaleString()} orders). Network customer rating holds at <strong>${totals.avgRating} / 5.0 ★</strong>.
      `;
    }
    ...
    if (bgOrders) {
      bgOrders.innerHTML = `
        <strong style="font-size:0.92rem;">Peak Volume & Fulfillment Insights (${periodLbl}):</strong><br>
        Active order volume for <strong>${periodLbl}</strong> is <strong>${totals.totalOrders.toLocaleString()} orders</strong> (${activeBranch}). Peak ordering rush occurs at <strong>12:00 PM – 2:00 PM</strong> (Lunch) and <strong>6:00 PM – 9:00 PM</strong> (Dinner Peak), with a <strong>98.3% order completion rate</strong>.
      `;
    }
    ...
    if (bgRevenue) {
      bgRevenue.innerHTML = `
        <strong style="font-size:0.92rem; color:var(--text-primary);">Financial Analysis & Marketplace Fee Insights (${periodLbl}):</strong><br>
        Gross item sales for <strong>${periodLbl}</strong> reached <strong>CAD $${parseFloat(totals.totalSales).toLocaleString()}</strong> (${activeBranch}). After marketplace commission fees of <strong>CAD -$${parseFloat(totals.totalFees).toLocaleString()}</strong> (${feePct}% fee rate), net payout to Ali Baba's chain was <strong>CAD $${parseFloat(totals.totalRevenue).toLocaleString()}</strong>.
      `;
    }
    ...
    if (bgDowntime) {
      ...
      bgDowntime.innerHTML = `
        <strong style="font-size:0.92rem;">Key Downtime Cause Insights (${periodLbl}):</strong><br>
        Logged offline duration for <strong>${periodLbl}</strong> totaled <strong>${dtHours.toLocaleString()} hours</strong> (${activeBranch}). Tablet disconnections account for 53% of offline time, followed by missed-order Uber Eats auto-pauses at Kipling Ave and Bloor & Islington.
      `;
    }
    ...
    if (bgRatings) {
      bgRatings.innerHTML = `
        <strong style="font-size:0.92rem; color:var(--text-primary);">Customer Review & Item Rating Insights (${periodLbl}):</strong><br>
        Average customer rating across <strong>${activeBranch}</strong> is <strong>${totals.avgRating} / 5.0 ★</strong> for <strong>${periodLbl}</strong>. Kipling Ave (5.0 ★) and Danforth (4.86 ★) lead customer review satisfaction. Falafel Wrap and Beef Shawarma Wrap received 100% 5-star ratings.
      `;
    }
    ...
    if (bgAccuracy) {
      bgAccuracy.innerHTML = `
        <strong style="font-size:0.92rem;">Order Accuracy & Top Item Inaccuracies Insights (${periodLbl}):</strong><br>
        Total order inaccuracy reports for <strong>${periodLbl}</strong> stand at <strong>${totals.totalInaccurate} cases</strong> (${activeBranch}). Missing items represent 60.8% of cases, with <strong>Chicken Shawarma Wrap</strong> and <strong>Garlic Sauce (Medium Side)</strong> accounting for the highest issue rates.
      `;
    }
  }
